...
const validCoord=h=>Number.isFinite(Number(h?.lat))&&Number.isFinite(Number(h?.lng))&&Number(h.lat)>=20&&Number(h.lat)<=46&&Number(h.lng)>=122&&Number(h.lng)<=154;
...
function loadStaticData(){
  ...
  if(allHospitals.length){
    ...
    draw();
    const pts=allHospitals.filter(validCoord).map(h=>[Number(h.lat),Number(h.lng)]);
...
async function geocode(h){if(validCoord(h))return true;try{const q=encodeURIComponent((h.pref? h.pref+" ":"")+h.address+" "+h.name);const r=await fetch(`https://nominatim.openstreetmap.org/search?format=json&limit=1&countrycodes=jp&q=${q}`,{headers:{"Accept-Language":"ja"}});const j=await r.json();if(!j?.length)return false;h.lat=Number(j[0].lat);h.lng=Number(j[0].lon);return true}catch(e){return false}}
async function ensureNearbyCoords(){
 if(!incident)return;
 const missing=allHospitals.filter(h=>!validCoord(h)&&visible(h));
 for(let i=0;i<missing.length;i+=5)await Promise.all(missing.slice(i,i+5).map(geocode));
}

function filtered(){
 let a=allHospitals.filter(visible).filter(validCoord).map(h=>({...h,lat:Number(h.lat),lng:Number(h.lng),dist:incident?hav(incident,{...h,lat:Number(h.lat),lng:Number(h.lng)}):null}));
...
function facilityKey(h){return `${h.name}||${h.address||""}`}
function render(arr=filtered()){
 const mode=sortMode.value;
 arr=arr.map((h,i)=>({...h,_i:i,_key:facilityKey(h),road:roadTimes[facilityKey(h)],roadKm:roadDistances[facilityKey(h)],air:incident?airMin(h.dist):null}));
...
async function routeFor(h,geometry=false){if(!validCoord(h))throw new Error("invalid coordinate");const u=`https://router.project-osrm.org/route/v1/driving/${incident.lng},${incident.lat};${h.lng},${h.lat}?overview=${geometry?"full":"false"}&geometries=geojson`;const r=await fetch(u),j=await r.json(),rt=j.routes?.[0];if(!rt)throw 0;return rt}
calcBtn.onclick=async()=>{if(!incident)return alert("先に発災地点を指定してください。");await ensureNearbyCoords();const arr=filtered();roadTimes={};roadDistances={};let done=0;for(let i=0;i<arr.length;i+=4){await Promise.all(arr.slice(i,i+4).map(async h=>{try{const rt=await routeFor(h,false);roadTimes[facilityKey(h)]=rt.duration/60;roadDistances[facilityKey(h)]=rt.distance/1000;done++}catch(e){}}));status.textContent=`陸路計算中… ${Math.min(i+4,arr.length)}/${arr.length}`;draw()}status.textContent=`${done}施設の陸路を計算しました。`};
async function showRoute(key){if(!incident)return;const h=allHospitals.find(x=>facilityKey(x)===key);if(!h)return;try{const rt=await routeFor(h,true);if(routeLayer)map.removeLayer(routeLayer);routeLayer=L.geoJSON(rt.geometry,{style:{weight:5,opacity:.8}}).addTo(map);map.fitBounds(routeLayer.getBounds(),{padding:[30,30]});roadTimes[facilityKey(h)]=rt.duration/60;roadDistances[facilityKey(h)]=rt.distance/1000;draw();status.textContent=`${h.name}: 陸路 約${Math.round(rt.duration/60)}分 / ${(rt.distance/1000).toFixed(1)}km`}catch(e){status.textContent="ルート取得に失敗しました。"}}
